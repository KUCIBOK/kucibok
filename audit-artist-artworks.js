#!/usr/bin/env node
/**
 * audit-artist-artworks.js — Audit de la DB Supabase pour trouver la source du bug 0→300
 *
 * Usage: node audit-artist-artworks.js <artist_id>
 * Example: node audit-artist-artworks.js fe40be9f-1f65-42ed-8743-ed5b3f89d448
 */
...
if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) {
  ...
  console.error('   Check your .env or Vercel environment variables')
  process.exit(1)
}
...
if (!artistId) {
  ...
  console.error('Usage: node audit-artist-artworks.js <artist_id>')
  process.exit(1)
}
  ...
  try {
    ...
    if (artworks && artworks.length > 0) {
      ...
      if (artworks.length === 0) {
        ...
        console.log('\n   ACTION NEEDED:')
        console.log('   - Verify these 300 artworks in Supabase dashboard')
        console.log('   - Check if they should belong to this artist')
        console.log('   - If NO: This is a data migration bug that needs fixing')
        console.log('   - If YES: Check why they all have same artist_id (may be legitimate)')
      } else {
        console.log(`⚠️  SUSPICIOUS: Artist has ${artworks.length} artworks (unexpected number)`)
      }

      // 5️⃣ Check if artworks have user_id mismatch
